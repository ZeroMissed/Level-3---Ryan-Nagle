...
public class LongChipCompetition {
	...
	public static void main(String[] args) {
		...
		comp.findLongestChip();
	}
	private void findLongestChip() {
		double longestChip = 0;
		Beatle beatleWLongChip = null;
		for(Beatle a : theBeatles){
			for(Chip b :  a.getChips()) {
				if(b.getLength() > longestChip) {
					longestChip = b.getLength();
					beatleWLongChip = a;
				}
			}
		}
		System.out.printf("The beatle with the longest chip is %s with a length of %1.3f!", beatleWLongChip.getName(), longestChip);
	}
