...
public class MurderOfCrows {
	...
	public static void main(String[] args) {
		float totalCrowsDead = 0;
		int numberOfTimesToAverage = 100;
		for(int d = 0; d < numberOfTimesToAverage; d++) {
		
		MurderOfCrows murderOfCrows = new MurderOfCrows();
		murderOfCrows.initializeCrows();
		totalCrowsDead += murderOfCrows.findTheDiamond();
	}
		System.out.println("On average, " + totalCrowsDead/numberOfTimesToAverage + " had to die to find the diamond!");
	}
	private int findTheDiamond() {
		/*
		 ...
		 */
		int crowsDead = 0;
		Ryan : for(Crow a : theMurder) {
			crowsDead++;
			for(String c : a.getStomachContents()) {
				if(c.equals("diamond")) {
					break Ryan;
				}
			}
